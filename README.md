...
- выполнять бинарные файлы прошивки (форматы .elf или .bin) для STM32F103C8T6 в среде Linux (или WSL) без наличия физической платы;
...
- **Таймер общего назначения (TIM2 или TIM3)**:
...
- Поддержка одновременного выполнения до 10 экземпляров ядра на серверной конфигурации.
